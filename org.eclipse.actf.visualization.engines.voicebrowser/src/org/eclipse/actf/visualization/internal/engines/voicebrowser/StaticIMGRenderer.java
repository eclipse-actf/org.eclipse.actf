...
public class StaticIMGRenderer implements IElementRenderer {
	...
	public PacketCollection getPacketCollectionIn(Element element, Context curContext, String url,
		...
		*/
		...
		if (altstr.length() == 0) {
			if (OutLoud.jwat_mode == IVoiceBrowserController.SCREEN_READER_MODE) {
				...
				for (;;) {
					...
					if (node.getNodeType() == Node.ELEMENT_NODE && node.getNodeName().toLowerCase().equals("body")) //$NON-NLS-1$
						return null;
					if (node.getNodeType() == Node.ELEMENT_NODE && node.getNodeName().toLowerCase().equals("a")) //$NON-NLS-1$
						break;
		...
		// build result string
		String result = OutLoud.buildResultString(mc, url, element, null, null, "<name=str1>", //$NON-NLS-1$
				altstr);
